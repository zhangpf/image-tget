...
class BTProtocol(protocol.Protocol):
    ...
    def finish_handshake(self):
        ...
        self.upload.start()
        self.__upload_monitor = self.upload._upload_monitor
        self.download.start()
        self.__download_monitor = self.download._download_monitor

        self.send_bitfield(self.btm.piece_manager.bitfield)
        
        self.send_keep_alive()

        if self.btm.connection_manager.is_already_connected(self.peer_id) :
            reactor.callLater(0, self.transport.loseConnection)
        else:
            logging.info('Add a new active connection %s' % self.peer_id)
            self.factory.add_active_connection(self.peer_id, self)
    ...
    def connectionLost(self, reason=None):
        if self.status == 'started':
            ...
            del self.__upload_monitor
            del self.__download_monitor
            del self.upload
            del self.download
            del self.btm
            
        self.factory.remove_active_connection(self)
    ...
    def send_message(self, _type, data):
        ...
        self.__upload_monitor(_type, data)

    def __upload_monitor(self, _type, data):
        pass
    ...
    def __download_monitor(self, data):
        pass
    
    def dataReceived(self, data):
        self.__download_monitor(data)
...
class BTServerProtocol (BTProtocol):
    ...
    def post_handshake(self):
        factory = self.factory.reset_factory(self, self.peer_info_hash)
        if factory:
            self.send_handshake()
        else:
            reactor.callLater(0, self.transport.loseConnection)
